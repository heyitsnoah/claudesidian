...
async function convertPdfToImages(pdfPath, outputDir, options = {}) {
  const { scale = 2.0, format = 'png', prefix = 'page-' } = options;
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  ...
  if (args.length < 2) {
    console.log('Usage: node pdf-to-images.mjs <pdf-path> <output-dir> [options]');
    console.log('Options:');
    console.log('  --scale <number>    Scale factor (default: 2.0)');
    console.log('  --format <string>   Output format: png or jpeg (default: png)');
